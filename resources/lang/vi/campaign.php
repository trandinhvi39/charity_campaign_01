<?php

return [
    'index' => '#',
    'list' => 'Danh sách chiến dịch',
    'create' => 'Tạo chiến dịch từ thiện',
    'create_success' => 'Tạo chiến dịch thành công',
    'name' => 'Tên chiến dịch',
    'start_date' => 'Ngày bắt đầu',
    'end_date' => 'Ngày kết thúc',
    'image' => 'ảnh',
    'address' => 'Địa điểm',
    'description' => 'Mô tả',
    'status' => 'Trạng thái',
    'action' => 'Hành động',
    'by' => 'Bởi',
    'contribute' => 'Đóng góp',
    'create_contribute' => 'Tạo đóng góp',
    'category' => 'Danh mục',
    'choose_one' => 'Vui lòng chọn một danh mục',
    'amount' => 'Số lượng',
    'contributors' => 'Những đóng góp',
    'show_more' => 'Xem chi tiết',
    'list_contribution' => 'Danh sách đóng góp',
    'contribution' => [
        'index' => '#',
        'avatar' => 'Ảnh đại diện',
        'name' => 'Tên',
        'email' => 'Địa chỉ email',
        'category' => 'Danh mục',
        'amount' => 'Số lượng',
        'description' => 'Mô tả',
        'status' => 'Trạng thái',
        'time' => 'Thời gian',
    ],
    'create_comment' => 'Tạo bình luận',
    'your_message' => 'Bình luận của bạn',
    'comments' => 'Bình luận',
    'value' => 'Những đóng góp hiện tại',
    'request_join' => 'Tham gia',
    'request_sent' => 'Yêu cầu đã đuợc gửi',
    'leave_campaign' => 'Rời',
    'approve' => 'Chấp nhận',
    'remove' => 'Xóa',
    'activated' => 'Đang hoạt động',
    'not_active' => 'Chưa hoạt động',
    'confirm' => 'Xác nhận',
    'active' => 'Hoạt động',
    'close' => 'Đóng',
    'categories' => 'Những danh mục đóng góp',
    'total' => 'Tổng',
    'error' => 'Lỗi',
    'must_join_campaign' => 'Bạn phải tham gia chiến dịch này',
    'star' => 'Sao',
    'one_star' => 'Một',
    'two_star' => 'Hai',
    'three_star' => 'Ba',
    'four_star' => 'Bốn',
    'five_star' => 'Năm',
    'rating' => 'Đánh gía',
    'goal' => 'Mục tiêu',
    'validate' => [
        'required_input' => 'Bạn phải chọn một danh mục và đơn vị',
        'amount' => 'Amount invalid',
        'contribution_type' => [
            'contribution' => 'Contribution type',
            'required' => 'đóng góp là bắt buộc',
            'minlengh' => 'Vui lòng nhập ít nhất 5 ký tự',
        ],
        'name' => [
            'required' => 'Tên chiến dịch là bắt buộc',
            'minlengh' => 'Vui lòng nhập ít nhất 10 kí tự',
        ],
        'goal' => [
            'goal' => 'Goal',
            'required' => 'Mục tiêu là bắt buộc',
            'number' => 'VUi lòng nhập số hợp lệ',
        ],
        'unit' => [
            'unit' => 'Unit',
            'required' => 'đơn vị là bắt buộc',
        ],
        'image' => [
            'required' => 'ảnh là bắt buộc',
        ],
        'start_date' => [
            'start_date' => 'Start date',
            'required' => 'Ngaỳ bắt đầu là bắt buộc',
        ],
        'end_date' => [
            'end_date' => 'End date',
            'required' => 'Ngaỳ kết thúc là bắt buộc',
        ],
        'location' => [
            'required' => 'Địa điểm là bắt buộc'
        ],
        'description' => [
            'required' => 'Mô tả là bắt buộc'
        ]
    ],
    'create_contribute_success' => 'Tạo đóng góp thành công',
    'upload_image_success' => 'Đã tải ảnh lên thành công',
    'upload_image_error' => 'Tải lên lỗi',
    'must_login' => 'Ngưòi dùng phải đăng nhập',
    'message_confirm' => 'Bạn có chắc chắn không?',
    'author' => 'Người tạo',
    'unit' => 'Đơn vị',
    'create_campaign' => 'Tạo chiến dịch từ thiện',
    'share_facebook' => 'Chia sẽ Facebook',
    'action_active_campaign' => 'active chiến dịch',
    'action_contribute_campaign' => 'đóng góp cho chiến dịch',
    'campaigns' => 'Số luợng chiến dịch',
    'message_end_campaign' => 'Còn :time trưóc khi chiến dịch kết thúc.',
    'show_detail' => 'Xem chi tiết',
    'confirmed' => 'Xác nhận',
    'waiting' => 'đang chờ',
    'unconfirmed' => 'Chưa xác nhận',
    'search_campaign' => 'Tìm kiếm chiến dịch',
    'members' => 'Thành viên',
    'contributor' => 'Ngưòi đóng góp',
    'progress' => 'Tiến độ',
    'activities' => 'Hoạt động',
    'users' => 'Người dùng',
    'event' => 'Sự kiện',
    'schedules' => 'Kế hoạch - Lịch trình',
    'label' => [
        'search' => 'Tìm kiếm thông tin chiến dịch từ thiện...',
        'STT' => 'Số thứ tự',
        'name' => 'Tên',
        'description' => 'Mô tả',
        'image' => 'Hình ảnh',
        'address' => 'địa chỉ',
        'start_time' => 'Ngày bắt đầu',
        'end_time' => 'Ngày kết thúc',
        'required' => '*',
    ],
    'panel_head' => [
        'index' => 'Danh sách chiến dịch từ thiện',
        'create' => 'Tạo chiến dịch từ thiện',
        'edit' => 'Chỉnh sửa chiến dịch từ thiện',
    ],
    'button' => [
        'create' => 'Tạo',
        'back' => 'Quay lại',
        'edit' => 'Chỉnh sửa',
        'reset_search' => 'Reset tìm kiếm',
        'search' => 'Tìm kiếm',
    ],
    'message' => [
        'confirm_delete' => 'Bạn có chắc chắn muốn xóa chiến dịch từ thiện này không?',
        'not_found_campaigns' => 'Không tìm thâý chiến dịch từ thiện nào trong hệ thống',
        'delete_success' => ' Xóa chiến dịch từ thiện thành công',
        'delete_fail' => ' Xóa chiến dịch từ thiện bị lỗi',
        'note' => 'Vui lòng nhập nội dung ghi chú',
    ],
    'tooltip' => [
        'show' => 'Xem chiến dịch từ thiện này',
        'delete' => 'Xóa chiến dịch từ thiện này',
    ],
    'placeholder' => [
        'name' => 'Vui lòng nhập tên chiến dịch từ thiện...',
        'desctiption' => 'Vui lòng nhập mô tả của chiến dịch từ thiện...',
        'address' => 'Vui lòng nhập địa chỉ chiến dịch từ thiện...',
        'start_time' => 'Vui lòng nhập ngày bắt đầu chiến dịch từ thiện...',
        'end_time' => 'Vui lòng nhập nagỳ kết thúc chiến dịch từ thiện',
        'note' => 'Nhập nội dung ghi chú',
    ],
    'label_for' => [
        'name' => 'Tên',
        'description' => 'Mô tả',
        'address' => 'địa chỉ',
        'start_time' => 'Ngày bắt đầu',
        'end_time' => 'Ngày kết thúc',
    ],
    'suggest_campaign' => 'Những chiến dịch từ thiện được đề xuất',
    'nearest' => 'Gần',
    'hotest' => 'Nổi bật',
    'edit' => 'Chỉnh sửa chiến dịch từ thiện',
    'save' => 'Lưu',
    'update_success' => 'Cập nhật chiến dịch từ thiện thành công',
    'update_error' => 'Cập nhật chiến dịch từ thiện lỗi',
    'closed' => 'Đã đóng',
    'old_image' => 'Ảnh cũ',
    'action_close_campaign' => 'đã đóng chiến dịch từ thiện này',
    'action_comment_campaign' => 'bình luận về chiến dịch từ thiện này',
    'filter_by' => 'Lọc theo',
    'newest' => 'Mới nhất',
    'oldest' => 'Cũ nhất',
    'open' => 'Đang hoạt động',
    'tags' => 'Tags cho chiến dịch',
    'related' => 'Liên quan',
    'notifications' => [
        'comment' => 'đã bình luận một chiến dịch của bạn',
        'approve' => 'đã chấp nhận yêu cầu của bạn',
        'confirm' => 'đã xác nhận đóng góp của bạn',
        'active' => 'đã mở lại chiến dịch này',
        'closed' => 'đã đóng chiến dịch này',
        'contribution' => 'đã đóng góp caò chiến dịch này',
        'join' => 'đã join vaò chiến dịch này',
    ],
    'popular_tags' => 'Danh mục từ thiện phổ biến',
    'all_tags' => ' Xem tất cả các tags',
    'tag' => [
        'name' => 'Tên tag',
        'count_campaigns' => 'Số lượng chiến dịch từ thiện',
    ],
     'create_note' => 'Tạo ghi chú cho chiến dịch này',
    'edit_note' => 'Chỉnh sửa ghi chú cho chiến dịch này',
    'btn_create_note' => 'Tạo ghi chú',
    'btn_edit_note' => 'Chỉnh sửa ghi chú',
    'note' => 'Ghi chú',
    'latest_update' => 'Thời gian cập nhật gần nhất',
    'by' => 'bởi',
    'no_notification' => 'Không có thông báo nào',
    'created' => 'Được tạo ',
];
